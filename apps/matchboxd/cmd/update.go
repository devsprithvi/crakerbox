...
var updateCmd = &cobra.Command{
	Use:   "update",
	Short: "Self-update crackerboxd to the latest version",
	Long: `Downloads and replaces the current crackerboxd binary with a newer version
from the official GitHub releases.

Examples:
  crackerboxd update                    # Update to latest stable
  crackerboxd update --version 0.2.0    # Update to specific version
  crackerboxd update --dry-run          # Check for updates without installing`,
	RunE: func(cmd *cobra.Command, args []string) error {
		repo := "devsprithvi/crakerbox"
		binaryName := "crackerboxd"
...
func fetchTagFromURL(url string) (string, error) {
	...
	rest := bodyStr[idx:]
	start := strings.Index(rest, `"`) + 1 // skip "tag_name"
	rest = rest[start:]
	start = strings.Index(rest, `"`) + 1 // skip :
	rest = rest[start:]
	start = strings.Index(rest, `"`) + 1 // skip opening quote of value
	rest = rest[start:]
